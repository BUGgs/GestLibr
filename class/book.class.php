...
Class Book {
    ...
    { 
        ...
        $this->secureData();
        if ($this->data["internet"] != 1) $this->data["internet"] = 0;
...
`old_value` ,`new_value` ,`comment`) VALUES (NULL ,  '".$this->idBook."',  '" . time() . "',  '".$_SESSION["idUser"]."',  'delete',  '',  '',  '')");
    ...
    function addCategory($idCategory,$idBook=-1)
    {
        ...
        if ($idBook==-1) $idBook=$this->idBook;
...
`old_value` ,`new_value` ,`comment`) VALUES (NULL ,  '-1',  '" . time() . "',  '".$_SESSION["idGroup"]."',  'disableCatalogOnInternet',  '',  '$idCatalog',  '')");
    ...
    */
	...
	$sql=$this->babase->DbSelect("SELECT * FROM `language` WHERE idGroup='".$_SESSION["idGroup"]."' ORDER by `languageName`",$nb);
        ...
        while($i!=$nb)
        {
            if (strlen($sql[$i]["languageName"])>30) $name = substr($sql[$i]["languageName"], 0, 30) . "...";
	    else $name=$sql[$i]["languageName"];
    ...
    */
	...
	$sql=$this->babase->DbSelect("SELECT * FROM `binding` WHERE idGroup='".$_SESSION["idGroup"]."' ORDER by `order`",$nb);
        ...
        while($i!=$nb)
        {
            if (strlen($sql[$i]["bindingName"])>30) $name = substr($sql[$i]["bindingName"], 0, 30) . "...";
	    else $name=$sql[$i]["bindingName"];
    ...
    */
	...
	$sql=$this->babase->DbSelect("SELECT * FROM `condition` WHERE idGroup='".$_SESSION["idGroup"]."' ORDER by `order`",$nb);
        ...
        while($i!=$nb)
        {
            if (strlen($sql[$i]["conditionName"])>30) $name = substr($sql[$i]["conditionName"], 0, 30) . "...";
	    else $name=$sql[$i]["conditionName"];
    ...
    */
	...
	$sql=$this->babase->DbSelect("SELECT * FROM `format` WHERE idGroup='".$_SESSION["idGroup"]."' ORDER by `order`",$nb);
        ...
        while($i!=$nb)
        {
            if (strlen($sql[$i]["formatName"])>30) $name = substr($sql[$i]["formatName"], 0, 30) . "...";
	    else $name=$sql[$i]["formatName"];
    ...
    */
	...
	if ($type==1)        
        ...
        while($i!=$nb)
        {
            if (strlen($sql[$i]["categoryName"])>50) $name = substr($sql[$i]["categoryName"], 0, 50) . "...";
	    else $name=$sql[$i]["categoryName"];
