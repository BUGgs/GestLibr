...
//        unset($tab);
	...
	function up($idCatalogData)
	{
		...
		if (!$this->islocked())
		{
			...
			else return(false); // idCatalogData Doesn't exist !
		}
  }
	...
	function getData($idCatalogData=-1,$start=0,$row=-1)
	{
		...
		settype($row, 'integer');
		if ($row==-1) $row=$this->defaultRow;
		
		if ($idCatalogData!=-1) $where=" AND idCatalogData='$idCatalogData'";
			else $where="";
	...
	function nbBooks($idCatalog=-1)
	{
		...
		if ($idCatalog==-1) $idCatalog=$this->idCatalog;
		settype($idCatalog, 'integer');
	...
	function getTemplate($idCatalogTemplate=-1)
	{
		...
		settype($idCatalogData, 'integer');
		if ($idCatalogTemplate!=-1) $where=" AND idCatalogTemplate='$idCatalogTemplate'";
		  else $where="";
...
//	die;
	...
	try
	{
	  $html2pdf = new HTML2PDF('P', 'A4', 'fr', true, 'UTF-8', array(12, 15, 12, 15));
